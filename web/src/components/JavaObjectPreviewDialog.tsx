...
const JavaObjectTreeNode = ({ depth, expanded, fieldName, node, path, toggle }: JavaObjectTreeNodeProps) => {
  ...
  return <>
    <div className={`java-object-row is-${node.kind}`} style={{ paddingLeft: `${12 + depth * 20}px` }}>
      ...
      <span className={`java-object-kind is-${fieldName ? "field" : "class"}`}>{fieldName ? "f" : "C"}</span>
...
export const JavaObjectPreviewDialog = ({ source, onClose }: JavaObjectPreviewDialogProps) => {
  ...
  useEffect(() => setExpanded(new Set(["root"])), [source]);
  ...
  return <div className="structured-preview-backdrop" onMouseDown={onClose}>
    <section ref={dialogRef} className="structured-preview-dialog java-object-preview-dialog" role="dialog" aria-modal="true" aria-label="Java 对象预览" tabIndex={-1} onMouseDown={(event) => event.stopPropagation()}>
      <header>
        <div><span>OBJECT INSPECTOR</span><strong>Java 对象预览</strong></div>
        <button type="button" aria-label="关闭 Java 对象预览" title="关闭" onClick={onClose}><CloseIcon /></button>
      </header>
      <div className="structured-preview-toolbar">
        <span>按字段查看对象结构与嵌套类型</span>
        <div>
      ...
      <div className="java-object-tree" role="tree">
        <JavaObjectTreeNode depth={0} expanded={expanded} node={root} path="root" toggle={toggle} />
      </div>
    </section>
  </div>;
};
